...
const StickerPeel = ({
  ...
  useEffect(() => {
    const target = dragTargetRef.current;
    if (!target || !draggable || disabled) return;
  ...
  return (
    <div
      className={`draggable ${peeled ? 'is-peeled' : ''} ${disabled ? 'sticker-disabled' : ''} ${className}`}
      ref={dragTargetRef}
      style={cssVars}
      role="button"
      tabIndex={disabled ? -1 : 0}
      aria-label={alt || 'Peel sticker'}
      aria-pressed={peeled}
      aria-disabled={disabled || undefined}
      onClick={() => !disabled && setPeeled(value => !value)}
      onKeyDown={event => {
        if (!disabled && (event.key === 'Enter' || event.key === ' ')) {
          event.preventDefault();
          setPeeled(value => !value);
        }
      }}>
